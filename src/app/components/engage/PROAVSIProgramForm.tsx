...
export default function SIProgramForm() {
  ...
  const [form, setForm] = useState<SIFormData>({
    company: "",
    contact: "",
    email: "",
    phone: "",
  ...
  const update = (field: keyof SIFormData, value: string) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="Capability">
        ...
        <Select label="Project Scale" value={form.projectScale} options={["Residential","Commercial","Enterprise","Multi-site"]} onChange={(v)=>update("projectScale",v)} />
      </Section>

      {/* TECHNICAL */}
      <Section title="Technical Capability">
        <Input label="Certifications (if any)" value={form.certifications} onChange={(v)=>update("certifications",v)} />
